...
def loss(flow_course, im1, im2, layer):
    ...
    im1_warped = tfcon.image.dense_image_warp(im1, flow)
    res = convnet(im1_warped, im2, flow, layer)
    flow_fine = res + flow

    im1_warped_fine = tfcon.image.dense_image_warp(im1, flow_fine)
    loss_layer = tf.reduce_mean(tf.math.squared_difference(im1_warped_fine, im2))
